...
class CustomerManageController extends Controller
{
    ...
    public function index(Request $request){
        if($request->keyword){
            $show_data = Customer::orWhere('customer_type', 'normal')->orWhere('phone',$request->keyword)->orWhere('name',$request->keyword)->paginate(20);
        }else{
             $show_data = Customer::where('customer_type', 'normal')->paginate(20);
        }
