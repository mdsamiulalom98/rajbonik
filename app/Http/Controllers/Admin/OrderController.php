...
class OrderController extends Controller
{
    ...
    public function index($slug, Request $request)
    {
        if ($slug == 'all') {
            ...
            $show_data = Order::where('customer_type' , 'normal')->latest()->with('shipping', 'status');
            if ($request->keyword) {
            ...
            $show_data = Order::where('customer_type' , 'normal')->where(['order_status' => $order_status->id])->latest()->with('shipping', 'status')->paginate(50);
        }
        $users = User::get();
    ...
    public function hostelorder_index($slug, Request $request)
    {
        if ($slug == 'all') {
            $order_status = (object) [
                'name' => 'All',
            ...
            $show_data = Order::where('customer_type' , 'hostel')->latest()->with('shipping', 'status');
            if ($request->keyword) {
                $show_data = $show_data->where(function ($query) use ($request) {
                    $query->orWhere('invoice_id', 'LIKE', '%' . $request->keyword . '%')
                        ->orWhereHas('shipping', function ($subQuery) use ($request) {
            ...
            $order_status = OrderStatus::where('slug', $slug)->withCount('orders')->first();
            $show_data = Order::where('customer_type' , 'hostel')->where(['order_status' => $order_status->id])->latest()->with('shipping', 'status')->paginate(50);
        }
        $users = User::get();
        $steadfast = Courierapi::where(['status' => 1, 'type' => 'steadfast'])->first();
        $pathao_info = Courierapi::where(['status' => 1, 'type' => 'pathao'])->select('id', 'type', 'url', 'token', 'status')->first();
        ...
        return view('backEnd.order.hostelorders', compact('show_data', 'order_status', 'users', 'steadfast', 'pathaostore', 'pathaocities'));
    }
