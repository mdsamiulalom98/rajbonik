...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $input['wholesale'] = $request->wholesale ? 1 : 0;
        if($request->type == 0){
    ...
    public function update(Request $request)
    {
        ...
        $input = $request->except(['image', 'product_type', 'files', 'sizes', 'colors', 'purchase_prices', 'old_prices', 'new_prices', 'stocks', 'images', 'up_id', 'up_sizes', 'up_colors', 'up_purchase_prices', 'up_old_prices', 'up_new_prices', 'up_stocks', 'up_images', 'pro_barcodes', 'up_pro_barcodes','up_wholesale_prices','wholesale_prices']);
        ...
        if ($request->up_id) {
            ...
            $up_wholesale_price    = $request->up_wholesale_prices;
            ...
            if ($update_ids) {
                foreach ($update_ids as $key => $update_id) {
                    ...
                    $upvariable->wholesale_price   = $up_wholesale_price[$key];
        ...
        if ($request->stocks) {
            ...
            $old_price  = $request->old_prices;
            $new_price  = $request->new_prices;
            $wholesale_price  = $request->wholesale_prices;
            $pro_barcode = $request->pro_barcodes;
            $images     = $request->file('images');
            if (is_array($stocks)) {
                foreach ($stocks as $key => $stock) {
                    ...
                    $variable->wholesale_price  = $wholesale_price[$key];
                    $variable->stock            = $stock;
