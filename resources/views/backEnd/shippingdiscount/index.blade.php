...
@section('title','Shipping Discount Manage') 
@section('css')
        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <a href="{{ route('shippingdiscount.create') }}" class="btn btn-primary waves-effect waves-light btn-sm rounded-pill">Create</a>
                </div>
                <h4 class="page-title">Shipping Discount Manage</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
                            ...
                            <tr style="vertical-align: middle;">
                                ...
                                <td>
                                    <div class="button-list">
                                        @if($value->status == 1)
                                        <form method="post" action="{{ route('shippingdiscount.inactive') }}" class="d-inline">
                                            @csrf
                                        ...
                                        @else
                                        <form method="post" action="{{ route('shippingdiscount.active') }}" class="d-inline">
                                            @csrf
                                            <input type="hidden" value="{{$value->id}}" name="hidden_id" />
                                        ...
                                        <a href="{{ route('shippingdiscount.edit',$value->id) }}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>

                                        <form method="post" action="{{ route('shippingdiscount.destroy') }}" class="d-inline">
                                            @csrf
                                            <input type="hidden" value="{{$value->id}}" name="hidden_id" />
