<form action="" method="POST">
        ...
        @foreach ($products as $value)
            ...
            @else
                @foreach ($value->variables as $variable)
                    <option value="{{ $value->id }}" data-size="{{ $variable->size }}" data-type="{{ $value->type }}"
                        data-color="{{ $variable->color }}" data-stock="{{ $variable->stock }}">
                        {{ $value->name }}
                        @if ($variable->size)
                            - {{ $variable->size }}
                        @endif
    ...
    <button class="mt-2 order_hostel" type="submit">Add</button>
